...
$$
   \Pr\{\text{decide} \; H_A^{(i,j)} | H_0^{(i,j) \; \text{true}}\} = \alpha^{(i,j)}.
$$
The family-wise type I error is larger than the specified pairwise errors $\alpha^{(i,j)}$.

**Tukey** controls the overall probability of a type I error across all pairwise comparisons,
$$
  \Pr\{\text{decide} \; H_A^{(i,j)} \; \text{for at least one pair} \; (i,j) | H_0 \; \text{true}\} = \alpha.
$$
The family-wise error $\alpha$ is specified, and thus the pairwise errors $\alpha^{(i,j)}$ are
adjusted ($\alpha^{(i,j)} < \alpha$).
