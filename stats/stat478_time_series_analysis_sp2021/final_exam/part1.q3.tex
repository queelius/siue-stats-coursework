...
\urlstyle{same}
\hypersetup{
  pdftitle={Time Series Analysis - STAT 478 - Final Exam - Part 1 Q3},
  pdfauthor={Alex Towell (atowell@siue.edu)},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Time Series Analysis - STAT 478 - Final Exam - Part 1 Q3}
\author{Alex Towell
...
\hypertarget{part-1-problem-3}{%
\section{Part 1: Problem 3}\label{part-1-problem-3}}
...
This is the form of an \(\ma(1)\) with \(\mu=β_1\). It is known that
\(\ma(1)\) is always stationary and has an ACF \[
  \rho_k =
  \begin{cases}
    1    & \text{if}\; |k| = 0\\
...
Now we do a case analysis on values of \(k\), but first note that by
symmetry \[
  \cov(\nabla Y_{t-k},\nabla Y_t) = \cov(\nabla Y_t,\nabla Y_{t-k})
\] and so we only consider non-negative values of \(k\).

\hypertarget{case-1-k0}{%
\subsubsection{\texorpdfstring{Case 1:
\(k=0\)}{Case 1: k=0}}\label{case-1-k0}}
...
\hypertarget{case-2-k1}{%
\subsubsection{\texorpdfstring{Case 2:
\(k=1\)}{Case 2: k=1}}\label{case-2-k1}}
...
\hypertarget{case-3-k-geq-2}{%
\subsubsection{\texorpdfstring{Case 3:
\(k \geq 2\)}{Case 3: k \textbackslash geq 2}}\label{case-3-k-geq-2}}

First, we consider \(k=2\) and generalize the result. The covariance of
\(\nabla Y_t\) and \(\nabla Y_{t-2}\) is given by \begin{align*}
  \cov(\nabla Y_t,\nabla Y_{t-2})
...
We see that the covariances are independent of time \(t\) and are only a
function of lag \(k\), and thus the \emph{autocorrelation} function is
...
variance is constant, and its autocorrelation function is strictly a
function of lag \(k\). Thus, \(\nabla Y_t\) is stationary.
